<?php
	...
	if(isset($_GET['datos'])){
		$datos = $_GET['datos'];
	list($accion, $dada, $idSorteo, $fecha, $jornada, $partido, $equipo1, $r1, $equipo2, $r2, $res, $jugado, $dia, $hora) = explode(",", $datos);
	// La variable acción nos permite saber que accion se ha de realizar
		switch ($accion) 
		{
			...
			case '4':
				...
				echo json_encode(EliminarSorteoQuinigol($idSorteo));
				break;
			
			case '5':
				// Se quiere eliminar el juego
				echo json_encode(ObtenerIdUltimoSorteo(9));
				break;
				case '6':
				// Se quiere eliminar el juego
				echo json_encode(ExisteFecha(9,$fecha));
				break;
			default:
				// Devolvemos error
				echo json_encode(-1);
		...
		$data = $_POST['datos'];
			...
			echo -1; // Error en los datos recibidos
				...
				if($data[0][2]==-1){ //Es un sorteo Nuevo -> corresponde al indice en el que se ha enviado el idSorteo
					$fecha = $data[0][4];
					$idSorteo = CrearSorteoQuinigol($fecha);
					...
					foreach ($data as $datos) {
						...
						$resultado = InsertarQuinigol( $idSorteo, $jornada, $partido, $equipo1, $r1, $equipo2, $r2, $res, $jugado, $dia, $hora);
					...
					foreach ($data as $datos) {
						...
						// Llama a la función InsertarQuiniela() con los datos
						$resultado =ActualizarQuinigol($idSorteo, $idQuinigol, $fecha, $jornada, $partido, $equipo1, $r1, $equipo2, $r2, $res, $jugado, $dia, $hora);
