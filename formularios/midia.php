<?php
	...
	if (isset($_POST['type'])) {
		...
		$datos = $_GET['datos'];
		list($accion, $idSorteo, $dia, $mes, $ano, $numero, $data) = explode (",", $datos);

		// La variable acción nos permite saber que acción se ha de realizar
		switch ($accion) {
			case '1':
				// Se quiere insertar el juego
				echo json_encode(InsertarSorteoMidia($dia, $mes, $ano, $numero, $data));
				break;

			case '2':
				// Se quiere actualizar el juego
				echo json_encode(ActualizarSorteoMidia($idSorteo, $dia, $mes, $ano, $numero, $data));
				break;

			case '4':
				// Se quiere eliminar el juego
				echo json_encode(EliminarSorteoMidia($idSorteo));
				break;
