<?php
	...
	if (isset($_POST['type'])) {
		...
		$datos = $_GET['datos'];
		list($accion, $idSorteo, $c1, $c2, $c3, $c4, $c5, $c6, $data) = explode (",", $datos);
		// La variable acción nos permite saber que acción se ha de realizar
		switch ($accion) {
			case '1':
				// Se quiere insertar el juego
				echo json_encode(InsertarSorteoQuintuple($c1, $c2, $c3, $c4, $c5, $c6, $data));
				break;

			case '2':
				// Se quiere actualizar el juego
				echo json_encode(ActualizarSorteoQuintuple($idSorteo, $c1, $c2, $c3, $c4, $c5, $c6, $data));
				break;

			case '4':
				// Se quiere eliminar el juego
				echo json_encode(EliminarSorteoQuintuple($idSorteo));
				break;
