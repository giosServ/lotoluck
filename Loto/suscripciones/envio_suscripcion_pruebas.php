...
$entregados = 0;
$errores = 0;
$id_estadistica = insertar_datos_estadisticas_ini('Pruebas', 'Manual');

//Recupera todos los correos de los suscriptores. En este caso se quiere acceder a la lista de pruebas.
$lista_suscriptores = obtenerSuscriptores_listaPruebas();

echo $lista_suscriptores[1];
$datos = array();
...
function obtenerSuscriptores_listaPruebas() {
    $consulta = "SELECT id_suscriptores FROM listas_correos WHERE id = 1;";
    // Se selecciona el string que almacena todos los ids de suscriptores almacenados en la lista de pruebas que tiene el id 1
    if ($resultado = $GLOBALS["conexion"]->query($consulta)) {
        while (list($string_ids) = $resultado->fetch_row()) {
            $arrayCadena = explode(',', $string_ids);
        }
        
        $lista = [];
        foreach ($arrayCadena as $id) {
